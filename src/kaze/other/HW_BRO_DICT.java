...
/*
 http://www.nowcoder.com/practice/03ba8aeeef73400ca7a37a5f3370fe68?tpId=37&tqId=21250&rp=&ru=/ta/huawei&qru=/ta/huawei/question-ranking
 */
public class HW_BRO_DICT {

    final static int ARR_SIZE = 32;

    static boolean isBro(String str, String ostr, final short[] obitmap) {
        if (str.length() != ostr.length()
                || ostr.equals(str)) return false;
        short bitMap[] = new short[ARR_SIZE];
        System.arraycopy(obitmap, 0, bitMap, 0, obitmap.length);
        char temp;
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNextInt()) {
            ...
            for (int i = 0; i < n; i++) {
                if(isBro(strs[i], str, bitMap)) {
                    list.add(strs[i]);
                }
            }
            Collections.sort(list);
            System.out.println(list.size());
            if(kth-1>= 0 && kth-1<list.size()) System.out.println(list.get(kth-1));
        }
    }
...
//    private static void randstr(StringBuffer str, int index) {
//        if(index == str.length()) {
